...
# estimate policy dvs tables ----------------------------------------------

tl_2 <- mdf %>% 
  filter(
    dvs %>% 
      is_in(
        c("dv_cc_enrgymix", "dv_cc_govdo")
      )
    ) %>% 
  arrange(dvs)
...
tl_2$r_m %>% 
  ...
  modelsummary::modelsummary(
    ...
    output = "gt",
    coef_map = tl_2$data[[1]]$cond_within %>% 
      levels %>%
      str_to_title %>% 
      c(
  ...
  gt::tab_spanner(label = 'Favor government redress Climate Change', columns = 8:13)
