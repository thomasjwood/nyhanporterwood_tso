...
ipw <- "https://github.com/thomasjwood/nyhanporterwood_tso/raw/main/t6.rds" %>%
  url %>% 
  gzcon %>% 
  readRDS %>% 
  rename(
...
m_2$s_m %>% 
  ...
  modelsummary::modelsummary(
    ...
    coef_map = m_2$data[[1]]$cond_within %>% 
      levels %>%
      str_to_title %>% 
      c(
  ...
  gt::tab_spanner(label = 'Favor renewable energy', 2:7) %>% 
  gt::tab_spanner(label = 'Favor govt redress climate', columns = 8:13)