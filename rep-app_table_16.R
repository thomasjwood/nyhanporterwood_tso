...
t1 %<>% 
  ...
  left_join(
    t1 %>% 
      ...
      select(workerid, ind_scitrust) %>% 
      mutate(
        ind_scitrust = ind_scitrust %>% 
          sjmisc::dicho() %>% 
          mapvalues(
            c(0:1),
            c("low",
...
mdf$l_m <- mdf %>%
  ...
  pmap(
    ...
    dvs, data
        ...
        dvs %>%
          str_c(
            " ~ pt_cc_cause  + pt_cc_enrgymix + import_cc + education + age + gender + ideol +  race + cont_polint  + region"
          ) %>%
          as.formula %>%
          cv.glmnet(
            alpha = 1,
...
mdf$frms <- mdf$dvs %>% 
  map2(
    ...
    str_c(
      ...
      " ~ cond_7 * ind_scitrust + ",
      j %>%
        str_replace_all("age45-54", "age") %>% 
        str_replace_all("raceNon-white", "race") %>% 
        str_c(collapse = " + ")
    ) %>% 
      as.formula
  )
