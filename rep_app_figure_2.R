...
p7 <- t4 %>%
  filter(
    dvs %>% 
      str_detect("Climate change", negate = F)
  ) %>% 
  mutate(
    c2 = c2 %>% 
      str_to_title %>% 
...
facet_grid(
  ...
  labs(
    ...
    subtitle = "Effects reported among participants who <span style = 'color:#059748'>**accept**</span> or <span style = 'color:#9D722A'>**reject**</span> anthropogenic climate change (pre-treatment)",
  ) +
  scale_x_discrete(
    labels = 2:4
    # breaks = NULL
